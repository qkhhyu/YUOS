...
| 阶段 | 状态 | 内容 | 目标 |
|------|------|------|------|
| 补强 1 | 🔲 待开始 | 修正任务栈模型：Handler 使用 MSP，任务使用 PSP | 让内核符合 Cortex-M RTOS 的标准异常模型，为真实项目打地基 |
| 补强 2 | 🔲 待开始 | 收紧任务状态机 | 调度器只选择 `READY` 任务，切走的 `RUNNING` 任务回到 `READY` 或进入 `BLOCKED` |
| 补强 3 | 🔲 待开始 | 加入 idle task | 所有业务任务阻塞时系统仍有合法任务运行，解决“阻塞任务被调度回来”的问题 |
| 补强 4 | 🔲 待开始 | 完善 `task_create()` 工程护栏 | 参数检查、栈 8 字节对齐、任务返回处理、栈溢出标记 |
| 实验 9 | 🔲 待开始 | `yield()` + 同优先级时间片轮转 | 学会主动让出 CPU，并让同优先级任务公平运行 |
| 实验 10 | 🔲 待开始 | 就绪队列与延时队列 | 从遍历 TCB 池过渡到链表/队列，理解 RTOS 数据结构骨架 |
...
*最后更新：2026-05-03（重整 RTOS 后续开发路线）*
